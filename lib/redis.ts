...
export async function getUserRating(
  ...
  return rating ? JSON.parse(rating as string) : null;
}
...
export async function getArticleStats(slug: string): Promise<ArticleStats> {
  ...
  const purchaseRecords = purchases as Record<string, string> || {};
  const ratingRecords = ratings as Record<string, string> || {};
  ...
  const allPurchases: PurchaseRecord[] = Object.values(purchaseRecords)
    .map((record) => JSON.parse(record) as PurchaseRecord)
  ...
  if (totalRatings > 0) {
    const sum = ratingValues.reduce(
      (acc, record) => acc + JSON.parse(record).score,
      0
    );
...
export async function getMultipleArticlesStats(
  ...
  slugs.forEach((slug, index) => {
    ...
    const purchases = (results[purchasesIndex] as Record<string, string>) || {};
    const ratings = (results[ratingsIndex] as Record<string, string>) || {};
